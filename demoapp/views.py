...
def predict_view(request):
    if request.method == 'POST':
        ...
        growth_type = request.POST.get('growth_type')
        ...
        # 🔹 Prediction for selected features
        for feature in selected_features:
            model = MODELS.get(feature, {}).get(state)
            if model:
                pred = model.predict([[year]])[0]
                result[feature] = round(pred, 2)
            else:
                result[feature] = None

        # 🔥 ALWAYS CALCULATE GROWTH RATE (FIXED)
        try:
            pop_model = MODELS.get('Population', {}).get(state)

            if pop_model:
                pop_this = pop_model.predict([[year]])[0]
                pop_prev = pop_model.predict([[year - 1]])[0]

                if pop_prev != 0:
                    growth = ((pop_this - pop_prev) / pop_prev) * 100
                    result['Population_Growth_Rate'] = round(growth, 2)
                else:
                    result['Population_Growth_Rate'] = 0
            else:
        ...
        # 🔹 Graph Data (first selected feature)
        selected_for_plot = selected_features[0]
        model = MODELS.get(selected_for_plot, {}).get(state)
